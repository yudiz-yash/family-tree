...
function computeLayout(nodes, selectedId, editMode) {
  ...
  const flowNodes = nodes.map((n) => {
    ...
    const isSelected = editMode && n.nodeId === selectedId;
    return {
      id: n.nodeId,
      type: 'person',
      position: positions[n.nodeId] || { x: 0, y: 0 },
      data: { node: n, isRoot, isSelected, editMode }
    };
  });
...
function PersonNode({ data }) {
  const { node, isRoot, isSelected, editMode } = data;
  ...
  const shadow = isSelected
    ? '0 0 0 3px #F4C430, 0 8px 30px rgba(244,196,48,0.4)'
    : isRoot
  ...
  return (
    <>
      ...
      <div style={{
        ...
        background: bg,
        border: `${isSelected ? 3 : 2.5}px solid ${border}`,
        borderRadius: 16, boxShadow: shadow,
        display: 'flex', flexDirection: 'column',
        alignItems: 'center', justifyContent: 'center',
        padding: '14px 12px', color: '#fff', textAlign: 'center',
        position: 'relative',
        ...
        {isSelected && (
          <div style={{
            ...
            padding: '2px 8px', borderRadius: 20, letterSpacing: 1, whiteSpace: 'nowrap'
          }}>SELECTED</div>
        )}
...
function Inner({ nodes, selectedId, editMode, onNodeSelect }) {
  const { flowNodes, flowEdges } = useMemo(
    () => computeLayout(nodes, selectedId, editMode),
  ...
  const onNodeClick = useCallback((_, rfNode) => {
    if (editMode && onNodeSelect) onNodeSelect(rfNode.id);
  }, [editMode, onNodeSelect]);
  ...
  return (
    <div style={{
      ...
      >
        ...
        <Controls style={{
          background: '#fff', border: '1px solid #e0d7f8',
          borderRadius: 10, boxShadow: '0 2px 8px rgba(0,0,0,0.1)'
        }} />
      </ReactFlow>
    </div>
  );
